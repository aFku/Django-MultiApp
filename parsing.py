...
#res = requests.get('https://www.cdaction.pl/')

#site = bs4.BeautifulSoup(res.text, "html.parser")

#print(site.find_all("div", id="flasher_left"))   #Karuzela

#x = site.find("div", id="newsy")
#print(x.find_all("div", {"class": "news"}))  # newsy z kolumny

##imdb.com

res = requests.get('https://www.imdb.com/news/movie')

site = bs4.BeautifulSoup(res.text, 'html.parser')

container = site.find("div", id="content-2-wide") # duzy container

news = site.find_all("article")      # wyciaganie artykułów

for art in news:
    print("-------------------------------------------------------")       # wszystko z tagiem article
    print(art)
